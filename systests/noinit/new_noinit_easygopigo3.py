...
# - Differs from (new May 2026) official easygopigo3.py
...
class EasyGoPiGo3(gopigo3.GoPiGo3):
    ...
    def __init__(self, config_file_path=None, use_mutex=False, noinit=False):
        """
        ...
        :var If noinit=False: int DEFAULT_SPEED = 300: Starting speed value: not too fast, not too slow.
...
def old_instantiation(fct_to_call,
    ...
    if gpg.use_mutex != use_mutex and not mutex.overall_mutex():
        gpg_mutex = "uses" if gpg.use_mutex else "does not use"
        sensor_mutex = "does" if use_mutex else "does not"
        raise ValueError(f"Invalid use of mutex: the GoPiGo3 {gpg_mutex} mutex protection and the {sensor_description} {sensor_mutex}.")

    msg = f"WARNING: Instantiating a {sensor_description} this way is deprecated."
...
def DHTSensor(gpg=None, sensor_type=0, use_mutex=False):
    ...
    if gpg.use_mutex != use_mutex:
        gpg_mutex = "uses" if gpg.use_mutex else "does not use"
        sensor_mutex = "does" if use_mutex else "does not"
        raise ValueError(f"Invalid use of mutex: the GoPiGo3 {gpg_mutex} mutex protection and the distance sensor {sensor_mutex}.")

    msg = "WARNING: Instantiating a DHTSensor this way is deprecated."
    warnings.warn(msg, DeprecationWarning)
    print(msg)
